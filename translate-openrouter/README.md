...
基于 OpenRouter 的 [Chat Completions](https://openrouter.ai/docs/api/api-reference/chat/send-chat-completion-request) 接口，将文本翻译为目标语言。

## 配置说明

- `请求地址`：默认为 `https://openrouter.ai/api/v1/chat/completions`
- `API Key`：接口访问令牌，**必填**
- `模型`：默认为 `openai/gpt-5.6-terra`
- `自定义模型`：选择 `自定义` 模型时使用
