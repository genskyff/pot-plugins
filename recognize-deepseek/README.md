...
基于 DeepSeek 的 [Chat Completions](https://api-docs.deepseek.com/zh-cn/api/create-chat-completion) 接口，对截图内容进行文本提取。

## 配置说明

- `请求地址`：默认为 `https://api.deepseek.com/chat/completions`
- `API Key`：接口访问令牌，**必填**
- `模型`：默认为 `deepseek-v4-flash-vision-exp`
- `自定义模型`：选择 `自定义` 模型时使用
- `温度`：为空或非数字时不发送
